                        ...
                        {mesas.map(m => (
                            <li key={m._id} className="flex items-center justify-between px-5 py-3">
                                <div className="flex items-center gap-3">
                                    <span className={`w-2 h-2 rounded-full shrink-0 ${m.activa ? "bg-emerald-500" : "bg-gray-300"}`} />
                                    <span className={`font-semibold ${m.activa ? "text-gray-900" : "text-gray-400 line-through"}`}>
                                        Mesa {m.nombre}
                                    </span>
                                    {!m.activa && <span className="text-xs text-gray-400">inactiva</span>}
                                </div>
                                <div className="flex items-center gap-1">
                                    <button onClick={() => toggleMesa(m._id)} className="p-2 rounded-lg hover:bg-gray-100 transition">
