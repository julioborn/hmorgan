            ...
            >
                ...
                <div className="mt-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <button onClick={() => setMetodoPago("efectivo")}
                            className={`relative py-4 rounded-xl border-2 transition flex flex-col items-center justify-center gap-1.5 ${metodoPago === "efectivo" ? "border-emerald-500 bg-emerald-50" : "border-gray-200 bg-white"}`}>
                            {metodoPago === "efectivo" && (
                ...
                <div className="flex gap-3 mt-4">
                    ...
                    <button
                        onClick={onEnviar}
                        disabled={enviando}
                        className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl font-bold text-base disabled:opacity-50 transition"
                    >
                        {enviando ? "Procesando..." : `Confirmar pedido · $${formatPrice(totalFinal)}`}
                    </button>
                </div>
            </motion.div>
        </motion.div>
...
export default function PedidosClientePage() {
    ...
    async function enviarPedido() {
        ...
        const { isConfirmed } = await swalBase.fire({
            ...
            html: `<p class="text-gray-600 text-sm">Total: <strong>$${formatPrice(totalFinalConfirm)}</strong>${tipoEntrega === "envio" ? " · Con envío a domicilio" : " · Retirás en el bar"}</p>${MERCADOPAGO_ACTIVO && metodoPago === "mercadopago" ? '<p class="text-blue-600 text-sm font-semibold mt-1">💳 Vas a pagar con Mercado Pago</p>' : metodoPago === "transferencia" ? '<p class="text-blue-600 text-sm font-semibold mt-1">🏦 Recordá transferir al alias <strong>morgan.bar</strong></p>' : ""}`,
            icon: "question",
            showCancelButton: true,
            confirmButtonText: MERCADOPAGO_ACTIVO && metodoPago === "mercadopago" ? "Ir a pagar" : "Sí, confirmar",
        ...
        return (
            <div className="bg-white min-h-screen pb-20">
                <div className="bg-white border-b border-gray-100 px-4 pt-4 pb-3">
                    <div>
                        <h1 className="text-2xl font-black text-gray-900">Realizar Pedido</h1>
                        <p className="text-xs text-gray-400 mt-0.5">Elegí una categoría para comenzar</p>
                    </div>
                </div>
